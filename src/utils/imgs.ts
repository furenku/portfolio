...
// For local files
export async function getBase64File(filePath: string): Promise<string> {
  try {
...
// For URLs
export async function getBase64Url(url: string): Promise<string> {
  try {
...
// For local files
export async function getImageDataFromFile(filePath: string): Promise<ImageData> {
  try {
    const imageBuffer = await fs.readFile(filePath);
    const metadata = await sharp(imageBuffer).metadata();
    const base64 = imageBuffer.toString('base64');
